...
export const CallListStats: React.FC<CallListStatsProps> = ({ calls, onExport }) => {
  ...
  return (
    <div className="w-full h-full p-4 overflow-y-auto bg-slate-50 custom-select-scroll" dir={direction}>
      <motion.div
      ...
      >
        ...
        {/* Header */}
        <motion.div variants={itemVariants} className="flex items-center gap-3 pb-4 border-b border-slate-200">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-slate-800">{tr('آمار جامع لیست', 'List Comprehensive Stats')}</h2>
            <p className="text-xs font-medium text-slate-500 mt-1">{tr('وضعیت شماره‌ها و تماس‌های این لیست', 'Status of numbers and calls in this list')}</p>
          </div>
          {onExport && (
            <button
              onClick={onExport}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-emerald-200 text-emerald-700 hover:bg-emerald-50 transition-all font-bold shrink-0"
            >
              <FileText size={18} />
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="bg-indigo-50 p-4 border border-indigo-100 flex flex-col gap-2 rounded-xl">
            <div className="flex items-center gap-2 text-indigo-600">
              <TrendingUp size={16} strokeWidth={2} />
              <span className="text-xs font-bold">{tr('کار شده امروز', 'Worked Today')}</span>
            </div>
            <span className="text-2xl font-bold text-indigo-700">{todayCount}</span>
          </div>

          <div className="bg-white p-4 border border-slate-200 flex flex-col gap-2 rounded-xl">
          ...
          <div className="bg-white p-4 border border-slate-200 flex flex-col gap-2 rounded-xl">
            ...
            <span className="text-2xl font-semibold text-teal-600">{stats.answered}</span>
          </div>
          ...
          <div className="bg-white p-4 border border-slate-200 flex flex-col gap-2 rounded-xl">
            ...
            <span className="text-2xl font-semibold text-rose-500">{stats.unanswered}</span>
          </div>
          ...
          <div className="bg-white p-4 border border-slate-200 flex flex-col gap-2 rounded-xl">
            ...
            <span className="text-2xl font-semibold text-orange-500">{stats.followUps}</span>
          </div>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Historical Calls Line Chart */}
          <motion.div variants={itemVariants} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col md:col-span-2">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
              <CalendarClock size={18} className="text-indigo-500" strokeWidth={2} />
              <h3 className="font-semibold text-sm text-slate-800">{tr('تاریخچه تماس‌های روزانه شما', 'Your Daily Call History')}</h3>
            </div>
          ...
          <motion.div variants={itemVariants} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
              <BarChart3 size={18} className="text-slate-500" strokeWidth={2} />
              <h3 className="font-semibold text-sm text-slate-800">{tr('تفکیک نتایج تماس', 'Call Results Breakdown')}</h3>
            </div>
            <div className="p-5 flex-1 min-h-[300px] h-[300px] relative">
          ...
          <motion.div variants={itemVariants} className="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm text-slate-800">{tr('نسبت نتایج کلی', 'Overall Ratio')}</h3>
            </div>
            <div className="p-5 flex-1 min-h-[300px] h-[300px] relative">
        ...
        {stats.topCourses.length > 0 && (
          <motion.div variants={itemVariants} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 bg-white flex items-center gap-2">
              <TrendingUp size={18} className="text-indigo-500" strokeWidth={2} />
              <h3 className="font-semibold text-sm text-slate-800">{tr('دوره‌های پرطرفدار در این لیست', 'Top Courses in List')}</h3>
            </div>
            <div className="p-0">
              <table className="w-full text-right">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    <th className="px-5 py-3 text-[11px] font-medium text-slate-500">{tr('نام دوره', 'Course Name')}</th>
                    <th className="px-5 py-3 text-[11px] font-medium text-slate-500 w-24 text-center">{tr('تعداد', 'Count')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {stats.topCourses.map((course, idx) => (
                    <tr key={course.name} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-5 py-3 text-xs font-medium text-slate-700 flex items-center gap-3">
                        <span className="w-5 h-5 rounded-md bg-indigo-50 text-indigo-600 flex items-center justify-center text-[10px]">{idx + 1}</span>
                        {course.name}
                      </td>
                      <td className="px-5 py-3 text-sm font-semibold text-indigo-600 text-center">{course.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
