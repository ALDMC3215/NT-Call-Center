              ...
              <div className="flex flex-col w-full pb-8">
                <div className="flex justify-end mb-4 px-4">
                  <button 
                    ...
                    disabled={isUpdating}
                    className="flex items-center gap-2 px-4 py-2 bg-brand-500/10 text-brand-500 hover:bg-brand-500/20 hover:text-brand-600 rounded-xl font-medium text-[12px] transition-colors border border-brand-500/20 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                <div className="hidden md:flex items-center px-4 py-3 mb-2 text-[12px] font-extrabold text-slate-500 uppercase tracking-widest border-b border-slate-200">
                  <div className="w-[35%] pl-4">{tr('مشخصات دوره', 'Course Details')}</div>
                  <div className="w-[20%] text-center">{tr('هزینه دوره', 'Price')}</div>
                  ...
                  {filteredCourses.map((course, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Course Info */}
                        <div className="w-full md:w-[35%] flex items-center gap-4 relative z-10">
                          <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 flex items-center justify-center group-hover:scale-105 group-hover:bg-brand-500 group-hover:text-white group-hover:border-brand-500 transition-all duration-500 shadow-sm shrink-0">
                            <CourseIcon size={20} strokeWidth={1.5} />
                          </div>
                          <div className="flex flex-col gap-1 flex-1 min-w-0">
                            <h4 className="text-[15px] font-extrabold text-slate-900 group-hover:text-brand-600 transition-colors truncate tracking-tight">{course.title}</h4>
                            <span className="text-[11px] font-medium text-slate-500 truncate">{course.subcategoryTitle}</span>
                          </div>
                        </div>

                        {/* Price */}
                        <div className="w-full md:w-[20%] flex items-center justify-center gap-2 text-slate-800 group-hover:text-brand-600 transition-colors relative z-10">
                          <span className="text-[15px] font-extrabold tracking-tight">{course.price || tr('نامشخص', 'Unknown')}</span>
                        </div>
